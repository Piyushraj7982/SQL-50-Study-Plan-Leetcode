...
from Activity),2) fraction from activity as a inner join
( select player_id, min(event_date) as min_event_date from Activity group by 1)
as b on a.player_id=b.player_id where datediff(a.event_date, b.min_event_date) = 1
```
