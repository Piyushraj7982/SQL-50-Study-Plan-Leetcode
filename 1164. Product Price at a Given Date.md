...
### Approach
We can find the last price before 2019-08-16 for products whose price has been
changed on or before this date simply by using:
...
not in(select distinct product_id from Products where change_date <='2019-08-16' )
...
select distinct product_id, 10 as price from 
products group by product_id having
min(change_date) > "2019-08-16"
union select product_id, new_price
from Products where 
